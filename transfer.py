...
if len(sys.argv) != 3 or '-?' in sys.argv:
    if len(sys.argv) != 1:
        ...
        print('  - batch.csv must be a CSV file containing entries of all the desired transfers in the format:')
        print('\n      AMOUNT,SPL_TOKEN_ADDRESS,RECIPIENT_WALLET_ADDRESS\n')
        print('    the first record in batch.csv will be ignored as it is assumed to be column info.\n')
        exit(-1)
    else:
...
try:
    ...
    logging.error('solana-cli not found in PATH. please ensure that solana-cli is installed and added to PATH')
    exit(-2)
...
logging.info('opened log file at ' + datetime.now().strftime('transfer %d-%m-%Y %H-%M-%S.log'))
...
for record in records:
    ...
    logging.info('sending ' + amount + ' of token ' + token + ' to recipient ' + recipient)
