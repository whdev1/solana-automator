...
{
    log_error('it appears that prompt-sync is not installed. please install with "npm install prompt-sync"');
    process.exit(-2);
}
...
if(process.argv.length != 4 || '-?' in process.argv)
{
    ...
    if(process.argv.length != 2)
    {
        ...
        console.log('  - batch.csv must be a CSV file containing entries of all the desired transfers in the format:');
        console.log('\n      AMOUNT,SPL_TOKEN_ADDRESS,RECIPIENT_WALLET_ADDRESS\n');
        console.log('    the first record in batch.csv will be ignored as it is assumed to be column info.\n');
        process.exit(-1);
    }
    else
    {
...
{
    log_error('solana-cli not found in PATH. please ensure that solana-cli is installed and added to PATH');
    process.exit(-4);
}
...
if(keypair_path != '')
    log_info('preserving keypair path of ' + keypair_path);
else
    log_info('no keypair path found, setting keypair path to ' + keypair_filepath)
...
var log_file_name = 'transfer ' + new Date().toJSON().split(':').join('-') + '.log';
var log_data = "";
...
for(let n = 1; n < records.length; n++)
{
    ...
    if(records[n].length == 0 || records[n][0] == '' || records[n][1] == '' || records[n][2] == '')
    {
    ...
    log_info('sending ' + amount + ' of token ' + token + ' to recipient ' + recipient);
    ...
    try
    {
        get_proc_output('spl-token', ['transfer', '--fund-recipient', token, amount, recipient])
    }
    catch(e)
    {
